    ...
    <g transform="translate(-14, -14)">
      ...
      >
        <path
          ...
          d="M175 152zm230 149l-170-53v117l42 32v32l-74-21l-75 21v-32l43-32V248L0 301v-42l171-107V35q0-14 9-23t22.5-9t23 9t9.5 23v117l170 107v42z"
        />
      </svg>
    </g>
  );
...
function Plane({
  ...
  const flightElapsed = plane.waiting
    ? (plane.waitEndTime - plane.startTime) / 1000
    : elapsed;
  const t = plane.waiting ? flightElapsed : elapsed;

  const rotOffset = Math.sin(t * micro.speed * 2 + index) * micro.rotAmplitude;
  const floatOffset =
    Math.sin(t * micro.speed * 1.7 + index * 1.3) * micro.floatAmplitude;
  const breath =
    0.9 + Math.sin(t * micro.speed + index * 0.7) * micro.breathAmplitude;

  const angle = plane.waiting ? 0 : plane.angle;

  return (
    <g>
      {!plane.waiting && <Trail path={plane.path} progress={plane.progress} />}
      <motion.g
        style={{
          x: plane.x,
